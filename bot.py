...
def fetch_manhwa_info(query):
    url = (
        f"https://api.comick.io/v1.0/search?type=comic&page=1&limit=8&q={query}&t=false"
    )
...
async def start(client, message):
    buttons = [
        [
            InlineKeyboardButton("👨‍💻 Dev", url="https://t.me/diamond_toki"),
...
async def manga_search(client, message):
    ...
    await message.reply_text(
        "Select a manga:", reply_markup=InlineKeyboardMarkup(buttons)
    )
...
async def manga_details(client, callback_query):
    ...
    if data and "comic" in data:
        ...
        link = (
            f"https://comick.io/comic/{comic['slug']}?lang=en"
            if comic.get("slug")
            else "N/A"
        )
        cover = (
            f'https://meo.comick.pictures/{comic["md_covers"][0]["b2key"]}'
...
"""
        ...
        buttons = [
            [InlineKeyboardButton("📖 Chapters", callback_data=f"chapters|{hid}|1")]
        ]
...
async def fetch_chapters(client, callback_query):
    ...
    if (
        not chapters_data
        or "chapters" not in chapters_data
        or not chapters_data["chapters"]
    ):
    ...
    chapter_buttons.append(
        [InlineKeyboardButton("📥 Full Page", callback_data=f"fullpage|{hid}|{page}")]
    )

    navigation_buttons = []
    if page > 1:
        navigation_buttons.append(
            InlineKeyboardButton("<<", callback_data=f"chapters|{hid}|{page-1}")
        )
    navigation_buttons.append(
        InlineKeyboardButton(">>", callback_data=f"chapters|{hid}|{page+1}")
    )
...
async def process_chapter_queue(user_id):
    async def process_chapter(match):
        ...
        try:
            response = await asyncio.to_thread(
                scraper.get, chapter_url, headers=headers
            )
            ...
            await bot.send_document(
                chat_id=callback_query.message.chat.id,
                document=pdf_path,
                caption=f"[CH-{chap_num}] {sanitized_title} [@Manga_Mania_Pro].pdf",
                thumb=image_files[0],
            )
    ...
    # Process all chapters in the queue concurrently
